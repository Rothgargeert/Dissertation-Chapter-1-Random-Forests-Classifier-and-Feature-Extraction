...
```{r}
rf2 <- randomForest(LLOV ~ ., data = balanced.data, importance = TRUE)
rf2
```
...
i=5
for (i in 3:8) {
  rf2 <- randomForest(LLOV ~ ., data = TrainSet, ntree = 1000, mtry = i, importance = TRUE)
...
```{r}
rf2 <- randomForest(LLOV ~ ., data = TrainSet, ntree = 1000, mtry = 4, importance = TRUE)
rf2
```
