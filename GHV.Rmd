...
```
```{r}
df5$GHV <- relevel(df5$GHV, ref = "Positive")
levels(df5$GHV)
```
...
```{r}
rf1 <- randomForest(GHV ~ ., data = balanced.data, importance = TRUE)
rf1
```
```{r}
...
i=5
for (i in 3:8) {
  rf1 <- randomForest(GHV ~ ., data = TrainSet, ntree = 1000, mtry = i, importance = TRUE)
...
```
```{r}
rf1 <- randomForest(GHV ~ ., data = TrainSet, ntree = 1000, mtry = 8, importance = TRUE)
rf1
```
```{r}
varImp(rf1)
